...
<form action="{{ route('reviews.update', $review->id) }}" method="POST">
    ...
    <div class="col-12">
        ...
        <select id="attraction_id" name="attraction_id" class="form-control"  required>
            ...
            @foreach ($attractions as $attraction)
                <option value="{{ $attraction->id }}" {{ $review->attraction->id ==
    ...
    <label>Name</label>
    <input type ="text" name="reviewer_name" class="form-control mb-3" required value="{{ $review->reviewer_name }}">
    @error('reviewer_name')
        <div class="text-danger">{{ $message }}</div>
    @enderror

    <label>Comment</label>
    <textarea name="comment" class="form-control mb-3" >{{ $review->comment }}</textarea>
    @error('comment')
        <div class="text-danger">{{ $message }}</div>
    @enderror
